...
class Monkey:
    def __init__(self, name, number, is_expr, lhs=None, rhs=None, op=None):
        ...
        self.parent = None
        self.is_human = False
        self.side_of_human = None

    def __repr__(self):
        return f"Monkey({self.name=}, {self.number=}, {self.is_expr=}, {self.lhs=}, {self.op=}, {self.rhs=}, {self.side_of_human=})"
...
for line in lines:
    ...
    if len(sp) == 2:  # just a number, monkey
        ...
        parents[lhs] = (nm, "left")
        parents[rhs] = (nm, "right")
        monkeys[name] = nm

root = monkeys["root"]
# assign parents of human
human = monkeys["humn"]
parent, direction = parents["humn"]
while parent is not None:
    parent.side_of_human = direction
    parent, direction = parents[parent.name]


curr, _ = parents["humn"]
while curr is not root:
    curr.solve()
    curr, _ = parents[curr.name]
