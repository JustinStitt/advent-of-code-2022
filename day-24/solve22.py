...
def bfs(starting_state, end_pos, blizzard_states, directional_blizzard_states):
    global grid
    best = 2**31
    # Q = [starting_state]
    Q = PriorityQueue()
    ...
    while not Q.empty():
        curr = Q.get()
        # if curr.pos in blizzard_states[curr.minute]:
        #     print("woah, this shouldn't be possible!")
        #     printState(curr, blizzard_states)
        #     print(blizzard_states[curr.minute])
        #     exit(1)
        #     continue  # eaten by a blizzard!
        # printState(curr, blizzard_states)
        if (curr.pos, directional_blizzard_states[curr.minute]) in cache:
            continue
        ...
        cache[(curr.pos, directional_blizzard_states[curr.minute])] = 1
        ...
        for neighbor in getNeighbors(curr, blizzard_states):
            new_state = State(neighbor, curr.minute + 1, path=curr.path + [neighbor])
            # calculate priority
...
def main():
    ...
    there = bfs(starting_state, end_pos, blizzard_states, directional_blizzard_states)
    mid_state = State(end_pos, there)
    back = bfs(mid_state, start_pos, blizzard_states, directional_blizzard_states)
    final_state = State(start_pos, back)
    there_again = bfs(
        final_state, end_pos, blizzard_states, directional_blizzard_states
    )
    print(f"{there=}, {back=}, {there_again=}")
